...
*UPDATE*: I later learned that instead of emulating the MIPS32 Little Endian virtual machine with QEMU, I can just run a binary that is compiled with the target architecture, e.g. `qemu-mipsel ./luac -l /usr/lib/lua/luci/model/crypto.lua`. It is still a learning experience for me as I was new to QEMU when I used.
