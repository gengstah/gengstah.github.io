...
```c
__int64 __fastcall CldSyncDisconnectRoot(__int64 a1, __int64 a2, _QWORD *a3)
{
  __int64 v5; // rbx
  _QWORD *v6; // rax
  ...
  Buffer = a2;
  v5 = 0LL;
  FltAcquirePushLockExclusiveEx(a1 + 8, 0LL);
  v6 = RtlLookupElementGenericTableAvl((PRTL_AVL_TABLE)(a1 + 0x10), &Buffer);
  if ( v6 )
    v5 = v6[1];
  FltReleasePushLockEx(a1 + 8, 0LL);
  if ( v5 )
  {
    CldSyncDisconnectRootByObject(v5, 0LL);   // Disconnect
    if ( WPP_GLOBAL_Control != (PDEVICE_OBJECT)&WPP_GLOBAL_Control
      && (HIDWORD(WPP_GLOBAL_Control->Timer) & 0x100) != 0
      && BYTE1(WPP_GLOBAL_Control->Timer) >= 4u )
    {
      WPP_SF_qd(WPP_GLOBAL_Control->AttachedDevice, 38LL, &WPP_69db2c92eaf0323eb14c6b31d757a425_Traceguids, a1, 0);
    }
    *a3 = v5;         // Return the object
  }
         ...
         && BYTE1(WPP_GLOBAL_Control->Timer) >= 4u )
  {
    ...
    WPP_SF_qd(WPP_GLOBAL_Control->AttachedDevice, 37LL, &WPP_69db2c92eaf0323eb14c6b31d757a425_Traceguids, a1, 0);
  }
  return 0LL;         // Always return 0 (success)
}
```
...
```
Note: I am working mostly on IDA Pro but the diff is from Ghidra so decompiled code differ.
