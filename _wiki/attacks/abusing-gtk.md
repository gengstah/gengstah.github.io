---
title: Abusing GTK
permalink: /wiki/attacks/abusing-gtk/
layout: single
author_profile: true
tags:
- airsnitch
...
The [GTK](/wiki/concepts/wifi-key-hierarchy/#gtk--group-temporal-key) is shared with every client on a BSSID. Anyone holding the GTK can produce a broadcast/multicast frame that the AP and every other client will accept as legitimately group-encrypted traffic. AirSnitch turns that into a **client-to-client unicast injection** by wrapping a unicast IP datagram inside a GTK-encrypted broadcast Wi-Fi frame.
...
3. **The AP can't filter what goes over the air.** Frames constructed and broadcast by the attacker do not traverse the AP's [bridge](/wiki/concepts/virtual-ports/), so any AP-side ACL on client-to-client traffic is bypassed.
...
A second AirSnitch test exists for the close cousin [Broadcast Reflection](/wiki/attacks/broadcast-reflection/) (`--c2c-broadcast`), which reaches the same outcome via the AP rather than directly over the air, and does **not** require knowing the GTK at all.
...
- **Per-client randomized GTK in every handshake** ([Passpoint DGAF Disable extended to FT/FILS/group-key/WNM](/wiki/defenses/group-key-randomization/)).
...
- [Passpoint flaws](/wiki/attacks/passpoint-flaws/), [Broadcast Reflection](/wiki/attacks/broadcast-reflection/)
