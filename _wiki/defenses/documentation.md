...
permalink: /wiki/defenses/documentation/
layout: single
author_profile: true
tags:
- airsnitch
...
The recurring failure mode in the NDSS'26 paper is: a vendor ships a "client isolation" checkbox; an admin enables it; the admin assumes guests can't reach main-network clients; in fact (depending on vendor), they can, via [gateway bouncing](/wiki/attacks/gateway-bouncing/) or [port stealing](/wiki/attacks/port-stealing/) or [GTK abuse](/wiki/attacks/abusing-gtk/). The fix isn't always more crypto. Sometimes the fix is *the vendor stating clearly which traffic the checkbox blocks*.
...
1. **Are clients in the guest network allowed to communicate with each other?** ([Intra-BSSID isolation](/wiki/concepts/client-isolation/).)
2. **Is traffic allowed between the guest and main network? In which direction?** Per [gateway bouncing](/wiki/attacks/gateway-bouncing/) and [port stealing](/wiki/attacks/port-stealing/), the answer is often "yes, despite the setting".
...
- **When client isolation is enabled on an open network, or a network with a shared WPA2-PSK or WPA3-SAE password**, display a warning that this provides no security benefit against a determined insider — see [machine-on-the-side](/wiki/attacks/machine-on-the-side/) and [rogue AP](/wiki/attacks/rogue-ap/). Vendors marketing client isolation as a security feature on home WPA2-PSK networks are misleading their users.
...
- Whoever runs the network should periodically run [`airsnitch.py`](/wiki/tools/airsnitch-cli/) against their own deployment. The output is the actual answer to the documentation questions above.
...
- [Tested devices](/wiki/devices/tested-devices/) — what each tested vendor actually documents.
