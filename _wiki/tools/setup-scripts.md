...
permalink: /wiki/tools/setup-scripts/
layout: single
author_profile: true
tags:
- airsnitch
...
`server_triggered_port_restoration/server_pong.py` and `client_ping.py` implement the trick used in [auxiliary techniques](/wiki/attacks/auxiliary-techniques/#server-triggered) — a fixed-cadence ping/pong between an attacker-controlled server and a client to force periodic re-learning of the gateway MAC.
