---
title: RADIUS
permalink: /wiki/airsnitch/concepts/radius/
layout: single
author_profile: true
tags:
- airsnitch
...
After EAP authentication succeeds, the RADIUS server derives the EAP MSK, returns it to the AP wrapped in a RADIUS `Access-Accept`, and the AP uses it as the PMK for the [4-way handshake](/wiki/airsnitch/concepts/handshakes/#4-way-handshake) with the client.
...
If the attacker manages to **intercept the AP's outbound RADIUS packets** — exactly what [uplink port stealing](/wiki/airsnitch/attacks/port-stealing/#uplink) achieves by spoofing the gateway MAC — the shared secret becomes the only defence. Two of its protections are weak:
...
- pair the rogue RADIUS with a [rogue AP](/wiki/airsnitch/attacks/rogue-ap/) advertising the same SSID, and harvest legitimate users' EAP credentials when they connect.
...
- [Rogue AP](/wiki/airsnitch/attacks/rogue-ap/)
- [MAC spoofing prevention](/wiki/airsnitch/defenses/spoofing-prevention/)
