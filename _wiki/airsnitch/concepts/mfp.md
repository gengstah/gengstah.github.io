...
permalink: /wiki/airsnitch/concepts/mfp/
layout: single
author_profile: true
tags:
- airsnitch
...
- **Unicast management frames** (deauthentication, disassociation, action frames) are protected by the [PTK](/wiki/airsnitch/concepts/wifi-key-hierarchy/) — same key as data frames.
- **Broadcast/multicast management frames** are *authenticated* (not encrypted) by the [IGTK](/wiki/airsnitch/concepts/wifi-key-hierarchy/). MFP introduces the IGTK exactly for this purpose.
...
| --- | --- |
| [Machine-on-the-side](/wiki/airsnitch/attacks/machine-on-the-side/) | MFP blocks unauthenticated deauth, so the attacker can't trivially disconnect the victim to capture a fresh handshake. Workaround: wait for a natural disconnect, or use channel-switch beacon trick. |
| [Rogue AP](/wiki/airsnitch/attacks/rogue-ap/) | MFP doesn't prevent the rogue AP itself; it only protects an existing association on the *real* AP. Once the victim roams, MFP is irrelevant. |
| [Passpoint flaws](/wiki/airsnitch/attacks/passpoint-flaws/) | The forged broadcast WNM-Sleep Response is authenticated under the *shared* IGTK that MFP uses. MFP is the very mechanism that makes the attack work. |
| [Abusing GTK](/wiki/airsnitch/attacks/abusing-gtk/) | Independent of MFP — GTK protects data frames, not management. |
| [Gateway Bouncing](/wiki/airsnitch/attacks/gateway-bouncing/), [Port Stealing](/wiki/airsnitch/attacks/port-stealing/), [Broadcast Reflection](/wiki/airsnitch/attacks/broadcast-reflection/) | Independent of MFP — happen above L2 cryptography. |
...
- [Machine-on-the-side](/wiki/airsnitch/attacks/machine-on-the-side/)
- [Passpoint flaws](/wiki/airsnitch/attacks/passpoint-flaws/)
