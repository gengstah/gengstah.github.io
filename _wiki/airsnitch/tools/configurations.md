...
permalink: /wiki/airsnitch/tools/configurations/
layout: single
author_profile: true
tags:
- airsnitch
...
To test WPA3-SAE, change `key_mgmt=WPA-PSK` to `key_mgmt=SAE` and add `ieee80211w=2`. WPA3-Personal mandates [MFP](/wiki/airsnitch/concepts/mfp/), so `ieee80211w=2`.
...
This is how you confirm that attacks like [GTK abuse](/wiki/airsnitch/attacks/abusing-gtk/), [Gateway Bouncing](/wiki/airsnitch/attacks/gateway-bouncing/), and [Port Stealing](/wiki/airsnitch/attacks/port-stealing/) work even when the victim and attacker have **different EAP identities** with their own credentials. (NDSS'26's two real-university tests in §VII-F exercise exactly this case.)
...
For [WPA3-PK](/wiki/airsnitch/concepts/wpa-versions/#wpa3-pk-wpa3-public-key) public-hotspot mode. Useful for confirming that *encryption*-layer attacks (machine-on-the-side, rogue AP) are blocked but *switching*-layer attacks are not.
...
- [airsnitch.py CLI](/wiki/airsnitch/tools/airsnitch-cli/) — the runtime that consumes these configs.
- [WPA versions](/wiki/airsnitch/concepts/wpa-versions/) — what each mode means.
