...
permalink: /wiki/airsnitch/defenses/macsec/
layout: single
author_profile: true
tags:
- airsnitch
...
- [Gateway Bouncing](/wiki/airsnitch/attacks/gateway-bouncing/) needs the L3 destination to be readable by the gateway.
- [Port Stealing](/wiki/airsnitch/attacks/port-stealing/) needs the AP's bridge to forward the cleartext to the wrong port.
- [GTK abuse](/wiki/airsnitch/attacks/abusing-gtk/) needs the OS to accept a unicast IP datagram inside a broadcast frame.
- [Broadcast Reflection](/wiki/airsnitch/attacks/broadcast-reflection/) needs the AP to re-encrypt cleartext under a different GTK.
...
- **[Rogue AP](/wiki/airsnitch/attacks/rogue-ap/)** — if the victim is induced to associate to a rogue AP, the rogue AP can refuse to negotiate MACsec. The victim then sees no MACsec peer; behaviour depends on the policy:
...
- **[Machine-on-the-side](/wiki/airsnitch/attacks/machine-on-the-side/)** for traffic that doesn't go through MACsec (e.g. broadcast ARP, DHCP). Those keep flowing in the clear at the Wi-Fi layer.
...
- Key management is its own problem. MACsec uses MKA (MACsec Key Agreement, IEEE 802.1X-2010) for dynamic keying, often backed by a RADIUS-like infrastructure. For a small enterprise, that may add more attack surface (see [RADIUS](/wiki/airsnitch/concepts/radius/)) than it removes.
...
[Defences index](/wiki/airsnitch/defenses/index/) shows MACsec stops every primary AirSnitch attack except Rogue AP (where it gives only partial protection, depending on policy). It's the highest-leverage single defence — but the deployment cost is real, and most networks should also enable [VLANs](/wiki/airsnitch/defenses/vlans/) and [group key randomization](/wiki/airsnitch/defenses/group-key-randomization/) as table-stakes, regardless.

## See also

- [Defences index](/wiki/airsnitch/defenses/index/)
- [WPA versions](/wiki/airsnitch/concepts/wpa-versions/) — MACsec layers on top of any WPA mode.
- [Centralised decryption](/wiki/airsnitch/defenses/centralized-decryption/) — alternative for enterprises where MACsec at the client is impractical.
