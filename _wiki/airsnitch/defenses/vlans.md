...
permalink: /wiki/airsnitch/defenses/vlans/
layout: single
author_profile: true
tags:
- airsnitch
...
- **[Gateway Bouncing](/wiki/airsnitch/attacks/gateway-bouncing/) stops at the VLAN boundary.** Guest VLAN's gateway routes guest traffic. It refuses to route a packet from the guest VLAN to the main VLAN unless a firewall rule explicitly allows it. The L2-vs-L3 isolation gap goes away because the L3 boundary is now real.
- **[Port stealing](/wiki/airsnitch/attacks/port-stealing/) loses its target.** The bridge serving the guest VLAN's BSSIDs has no entry for any main-VLAN MAC. The attacker can't steal what isn't there.
- **[Broadcast Reflection](/wiki/airsnitch/attacks/broadcast-reflection/) stops at the VLAN boundary.** The AP's broadcast fan-out runs per-VLAN. A broadcast frame originating in the guest VLAN never reaches the main-VLAN clients.
- **[GTK abuse](/wiki/airsnitch/attacks/abusing-gtk/) stays within the VLAN** if the network uses per-VLAN GTKs (`gtk-per-vlan` option on some gear). An insider in the guest VLAN can still attack other guests, but can't attack the main network.
...
- **[Machine-on-the-side](/wiki/airsnitch/attacks/machine-on-the-side/) and [Rogue AP](/wiki/airsnitch/attacks/rogue-ap/)** are unaffected by VLANs — they don't use the AP's data path.
...
If your gear supports unique GTK per VLAN, enable it. Otherwise [GTK abuse](/wiki/airsnitch/attacks/abusing-gtk/) within the guest VLAN still works against other guests. Better still: per-client randomized GTK across the board (see [group key randomization](/wiki/airsnitch/defenses/group-key-randomization/)).
...
- [Group key randomization](/wiki/airsnitch/defenses/group-key-randomization/) — the encryption-layer counterpart.
- [Spoofing prevention](/wiki/airsnitch/defenses/spoofing-prevention/) — additional belt to the VLAN braces.
- [MACsec](/wiki/airsnitch/defenses/macsec/) — when VLANs alone aren't enough.
- [airsnitch.py CLI](/wiki/airsnitch/tools/airsnitch-cli/) — how to run the verification tests.
