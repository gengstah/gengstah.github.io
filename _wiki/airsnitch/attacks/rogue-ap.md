...
permalink: /wiki/airsnitch/attacks/rogue-ap/
layout: single
author_profile: true
tags:
- airsnitch
...
The WPA3 cousin of [machine-on-the-side](/wiki/airsnitch/attacks/machine-on-the-side/). With WPA3-SAE, the [PMK](/wiki/airsnitch/concepts/wifi-key-hierarchy/#pmk--pairwise-master-key) is derived per session via the Dragonfly handshake and **cannot** be derived passively from another client's handshake. So observing the air buys you nothing. But the **passphrase is still shared**. So an attacker who knows the passphrase can simply:
...
[Client isolation](/wiki/airsnitch/concepts/client-isolation/) is enforced *on the AP*. Once the victim is associated to a different AP — the attacker's — there's no isolation to enforce. The attacker is an AP, not a peer.
...
- [WPA versions](/wiki/airsnitch/concepts/wpa-versions/)
- [Documentation defence](/wiki/airsnitch/defenses/documentation/) — vendors should warn that client isolation is meaningless under shared passphrases.
