...
permalink: /wiki/airsnitch/attacks/passpoint-flaws/
layout: single
author_profile: true
tags:
- airsnitch
...
[Passpoint DGAF Disable](/wiki/airsnitch/concepts/passpoint/) is supposed to give every client a **unique random GTK** so [GTK abuse](/wiki/airsnitch/attacks/abusing-gtk/) doesn't work. The catch: DGAF Disable only mandates randomization in the **4-way handshake**. Every other handshake that delivers a GTK was overlooked in the spec. AirSnitch attacks that gap.

There are two sub-flaws, distinguished by which key is the lever:

- **Non-randomized GTK in non-4-way handshakes.** The spec doesn't require randomization in the [group-key handshake](/wiki/airsnitch/concepts/handshakes/#group-key-handshake), [FT](/wiki/airsnitch/concepts/handshakes/#fast-bss-transition-ft-handshake-ieee-80211r), [FILS](/wiki/airsnitch/concepts/handshakes/#fast-initial-link-setup-fils-handshake-ieee-80211ai), or [WNM-Sleep Response](/wiki/airsnitch/concepts/handshakes/#wnm-sleep-request--response-ieee-80211v). So even when DGAF Disable is honoured at association, the *real* shared GTK leaks during any of these later events.
- **Non-randomized [IGTK](/wiki/airsnitch/concepts/wifi-key-hierarchy/#igtk--integrity-group-temporal-key).** Even if every GTK delivery is randomized, the shared IGTK lets an attacker forge broadcast management frames.
...
Once the attacker holds the shared GTK, they switch to standard [GTK abuse](/wiki/airsnitch/attacks/abusing-gtk/).
...
Listed under [README §5.3 "Manual tests"](/wiki/airsnitch/tools/airsnitch-cli/#manual-tests) as a possible future addition: "Testing whether the IGTK group key is randomized under client isolation."

## See also

- [Passpoint](/wiki/airsnitch/concepts/passpoint/) — the spec being attacked.
- [Handshakes](/wiki/airsnitch/concepts/handshakes/) — what each handshake delivers.
- [Abusing GTK](/wiki/airsnitch/attacks/abusing-gtk/) — what to do once you hold the real GTK.
