...
permalink: /wiki/airsnitch/devices/tested-devices/
layout: single
author_profile: true
tags:
- airsnitch
...
### [Abusing GTK](/wiki/airsnitch/attacks/abusing-gtk/) — same-network injection

Vulnerable: **all 11 devices** tested, in M↔M and G↔G directions. Universal. The shared-GTK design is not vendor-specific.

### [Gateway Bouncing](/wiki/airsnitch/attacks/gateway-bouncing/)
...
### [Downlink Port Stealing](/wiki/airsnitch/attacks/port-stealing/#downlink)
...
### [Uplink Port Stealing](/wiki/airsnitch/attacks/port-stealing/#uplink)
...
### [Machine-on-the-side](/wiki/airsnitch/attacks/machine-on-the-side/) (single-BSSID WPA2-PSK)

Vulnerable: all 7 home APs. Universal. (NDSS'26 §VII-D "Single-BSSID Attacks".)

### [Rogue AP](/wiki/airsnitch/attacks/rogue-ap/) (single-BSSID WPA2/3)
...
- **TP-Link EAP613 (with VLAN configuration)** — used as the testbed for [VLANs and firewall](/wiki/airsnitch/defenses/vlans/). With per-BSSID VLANs enabled, every AirSnitch primary attack is blocked. Without VLANs, vulnerable like everything else.

The only devices in the paper that demonstrate *complete* AirSnitch resistance are the ones with **explicit VLAN configuration**, and even then only for cross-VLAN attacks — within a VLAN, the [shared-passphrase attacks](/wiki/airsnitch/attacks/machine-on-the-side/) are still feasible.
...
- [Defences index](/wiki/airsnitch/defenses/index/) — what to enable on each device.
- [airsnitch.py CLI](/wiki/airsnitch/tools/airsnitch-cli/) — how to retest after firmware updates.
- [NDSS'26 paper](/wiki/airsnitch/sources/ndss2026-paper/) — full source.
