---
title: Kernel Exploit Primitives
permalink: /wiki/kernel/primitives/
layout: single
author_profile: true
tags:
...
> **Related:** [Architecture](/wiki/kernel/architecture/), [Pool Internals](/wiki/kernel/pool_internals/), [Mitigations](/wiki/kernel/mitigations/), Arbitrary Write  
...
See [Ioring](/wiki/kernel/ioring/) for full structures and [Cve 2024 30085](/wiki/cves/CVE-2024-30085/) for exploit chain.
...
See [Cve 2025 53136](/wiki/cves/CVE-2025-53136/) for full analysis.
...
- Set `PreviousMode = 0` → `NtWriteVirtualMemory` now writes to kernel addresses
...
**Real-world example**: CVE-2024-26170 (`cimfs.sys`) — `\Device\cimfs\control` had System/Admin-only DACL but no `FILE_DEVICE_SECURE_OPEN`. Any user could reach the IOCTL interface via `\\??\\CimfsControl\\anything`. See [Cve 2024 26170](/wiki/cves/CVE-2024-26170/).
...
```
**Why this matters**: Documented in the Nokoyawa/CVE-2023-28252 in-the-wild exploit as the Win11 AAR/AAW path. Shows named pipe objects serve double duty — both as grooming objects AND as a post-corruption AAR/AAW primitive.
See [Cve 2023 28252](/wiki/cves/CVE-2023-28252/).
...
**Prerequisites**: Must have corrupted a `_KALPC_RESERVE` pointer. Typically done via a pool overflow into an `_ALPC_HANDLE_TABLE` (see [Heap Grooming](/wiki/techniques/heap_grooming/) — ALPC handle table spray pattern).

See [Cve 2024 30085](/wiki/cves/CVE-2024-30085/) for complete flow.
...
See [Cve 2021 31969](/wiki/cves/CVE-2021-31969/) for complete implementation.
...
| Mitigation | Impact on LSTAR exploit | Bypass |
|-----------|------------------------|--------|
| SMEP | Blocks execution of user shellcode from ring 0 | CR4 gadget: `mov cr4, rax` (bit 20/21 clear); or use kernel-space shellcode only |
| SMAP | Blocks kernel accessing user-mode RSP during ROP | Set RFLAGS AC bit (bit 18) from user mode before `syscall` |
| KVA Shadow (KPTI) | First gadget must be in KVASCODE only | Disabled for Admin processes; or use KVASCODE-constrained gadget set |
| KPP | Monitors LSTAR — detects corruption | Restore LSTAR as very first shellcode instruction |
| HVCI | CR4 modification blocked by secure kernel | Cannot disable SMEP; must use kernel-space ROP only or KVASCODE trampoline |
...
See [Mitigations](/wiki/kernel/mitigations/) §BYOVD Vulnerability Class: Unrestricted WRMSR and §KVA Shadow for bypass details.  
See [Rop](/wiki/techniques/rop/) §IRETQ Kernel Entry Frame for stack layout code.
...
See [Cve 2024 21338](/wiki/cves/CVE-2024-21338/) for the complete exploit.
...
See [Cve 2024 30090](/wiki/cves/CVE-2024-30090/) for full exploit chain.
...
See [Cve 2024 38238](/wiki/cves/CVE-2024-38238/) for full exploit chain.
