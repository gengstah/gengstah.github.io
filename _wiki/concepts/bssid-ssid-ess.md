...
permalink: /wiki/concepts/bssid-ssid-ess/
layout: single
author_profile: true
tags:
- airsnitch
...
- [GTK / IGTK](/wiki/concepts/wifi-key-hierarchy/)
...
| **BSSID** | The unit of [virtual port](/wiki/concepts/virtual-ports/). Cross-BSSID port stealing is what makes downlink interception possible (NDSS'26 §V-B). |
...
- **Same BSSID** (`--same-bss`). Attacker and victim on the same virtual port. Useful for [intra-BSSID](/wiki/concepts/client-isolation/) tests, [GTK abuse](/wiki/attacks/abusing-gtk/), [gateway bouncing](/wiki/attacks/gateway-bouncing/) baseline.
- **Different BSSID, same physical AP** (`--other-bss`). Required for [downlink port stealing](/wiki/attacks/port-stealing/#downlink) (the attacker needs a separate port to be the destination of stolen traffic). Common case: attacker on guest BSSID, victim on main BSSID; or 2.4 GHz vs 5 GHz of the same SSID.
