...
permalink: /wiki/concepts/wpa-versions/
layout: single
author_profile: true
tags:
- airsnitch
...
- **Most exposed mode.** Any insider with the passphrase can derive any other client's PTK by sniffing their EAPOL frames → [machine-on-the-side](/wiki/attacks/machine-on-the-side/).
...
- Does **not** defeat [rogue AP](/wiki/attacks/rogue-ap/), because anyone with the passphrase can stand up a clone.
...
| --- | --- |
| [Abusing GTK](/wiki/attacks/abusing-gtk/) | GTK is shared in every WPA mode by default; the attack requires association, not a specific cipher |
| [Gateway Bouncing](/wiki/attacks/gateway-bouncing/) | Pure L3 attack; encryption is terminated at the AP |
| [Port Stealing](/wiki/attacks/port-stealing/) | Exploits the AP's bridge FDB after decryption |
| [Broadcast Reflection](/wiki/attacks/broadcast-reflection/) | Exploits the AP's broadcast-handling logic after decryption |
| [Inter-NIC relaying / port-restoration helpers](/wiki/attacks/auxiliary-techniques/) | Pure switching/forwarding manipulation |
...
- [Machine-on-the-side](/wiki/attacks/machine-on-the-side/), [Rogue AP](/wiki/attacks/rogue-ap/)
- [RADIUS](/wiki/concepts/radius/)
- [Configuration files](/wiki/tools/configurations/) for AirSnitch's `client.conf`, `eap.conf`, `multipsk.conf`, `saepk.conf`.
