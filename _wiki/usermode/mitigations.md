---
title: User-Mode Mitigations
permalink: /wiki/usermode/mitigations/
layout: single
author_profile: true
tags:
...
> **Related:** [Mitigations](/wiki/kernel/mitigations/), [Rop](/wiki/techniques/rop/), [Heap Internals](/wiki/usermode/heap_internals/)  
...
- See [Rop](/wiki/techniques/rop/) for full details
...
**Approach 2 — Multi-step chain through benign XFG-compliant functions** (CVE-2024-26230 / k0shl technique):
When the attacker can invoke a controlled vtable dispatch multiple times, a chain of legitimate API calls can be built up piece by piece to achieve code execution:
...
2. Call `VirtualAlloc` → allocates RWX memory at predictable 32-bit address
   ...
   - `flAllocationType` (arg3) is a pointer; exploit the integer overflow: choose offset so `(ptr + offset) mod 2^32 = 0x3000` (MEM_COMMIT|MEM_RESERVE)
